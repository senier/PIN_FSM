...
package RFLX.PIN_FSM.Config with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   type Virtual_Field is (F_Initial, F_Max_Retries, F_PIN, F_Final);

   subtype Field is Virtual_Field range F_Max_Retries .. F_PIN;
      ...
      record
         case Fld is
            when F_Initial | F_Final =>
               null;
            when F_Max_Retries =>
               Max_Retries_Value : RFLX.PIN_FSM.Retries_Base;
            when F_PIN =>
   ...
   function Get_Max_Retries (Ctx : Context) return RFLX.PIN_FSM.Retries with
     Pre =>
       Valid (Ctx, F_Max_Retries);
   ...
   procedure Set_Max_Retries (Ctx : in out Context; Val : RFLX.PIN_FSM.Retries) with
     Pre =>
       not Ctx'Constrained
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_Max_Retries)
       and then Field_Condition (Ctx, (F_Max_Retries, To_Base (Val)))
       and then Valid (To_Base (Val))
       and then Available_Space (Ctx, F_Max_Retries) >= Field_Size (Ctx, F_Max_Retries),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Max_Retries)
       and Get_Max_Retries (Ctx) = Val
       and Invalid (Ctx, F_PIN)
       and (Predecessor (Ctx, F_PIN) = F_Max_Retries
            and Valid_Next (Ctx, F_PIN))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Predecessor (Ctx, F_Max_Retries) = Predecessor (Ctx, F_Max_Retries)'Old
       and Valid_Next (Ctx, F_Max_Retries) = Valid_Next (Ctx, F_Max_Retries)'Old;
   ...
   procedure Set_PIN (Ctx : in out Context; Val : RFLX.PIN_FSM.PIN) with
     ...
     Post =>
       ...
       and Get_Max_Retries (Ctx) = Get_Max_Retries (Ctx)'Old
       and Context_Cursor (Ctx, F_Max_Retries) = Context_Cursor (Ctx, F_Max_Retries)'Old;
   ...
   type Structure is
      record
         Max_Retries : RFLX.PIN_FSM.Retries;
         PIN : RFLX.PIN_FSM.PIN;
      end record;
   ...
   procedure To_Context (Struct : Structure; Ctx : in out Context) with
     Pre =>
       ...
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_Max_Retries)
       and then Available_Space (Ctx, F_Max_Retries) >= 72,
     Post =>
       Has_Buffer (Ctx);
...
private
   ...
   function Valid_Context (Buffer_First, Buffer_Last : RFLX_Types.Index; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length; Message_Last : RFLX_Types.Bit_Length; Buffer : RFLX_Types.Bytes_Ptr; Cursors : Field_Cursors) return Boolean is
     ((if Buffer /= null then Buffer'First = Buffer_First and Buffer'Last = Buffer_Last)
      ...
      and then ((if
                 ...
                 then
                    (Valid (Cursors (F_Max_Retries))
                     and then Cursors (F_PIN).Predecessor = F_Max_Retries)))
      and then ((if Invalid (Cursors (F_Max_Retries)) then Invalid (Cursors (F_PIN))))
      and then (if
                   Structural_Valid (Cursors (F_Max_Retries))
                then
                   Cursors (F_Max_Retries).Last - Cursors (F_Max_Retries).First + 1 = RFLX.PIN_FSM.Retries_Base'Size
                   and then Cursors (F_Max_Retries).Predecessor = F_Initial
                   and then Cursors (F_Max_Retries).First = First
                   and then (if
                                Structural_Valid (Cursors (F_PIN))
                             then
                                Cursors (F_PIN).Last - Cursors (F_PIN).First + 1 = RFLX.PIN_FSM.PIN'Size
                                and then Cursors (F_PIN).Predecessor = F_Max_Retries
                                and then Cursors (F_PIN).First = Cursors (F_Max_Retries).Last + 1)));
   ...
   function Initialized (Ctx : Context) return Boolean is
     (Ctx.Message_Last = Ctx.First - 1
      and then Valid_Next (Ctx, F_Max_Retries)
      and then Field_First (Ctx, F_Max_Retries) mod RFLX_Types.Byte'Size = 1
      and then Available_Space (Ctx, F_Max_Retries) = Ctx.Last - Ctx.First + 1
      and then Invalid (Ctx, F_Max_Retries)
   ...
   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value) return Boolean is
     ((case Val.Fld is
          when F_Initial | F_Max_Retries | F_PIN =>
             True,
          when F_Final =>
             False));
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length is
     ((case Ctx.Cursors (Fld).Predecessor is
          when F_Initial =>
             (case Fld is
                 when F_Max_Retries =>
                    RFLX.PIN_FSM.Retries_Base'Size,
                 when others =>
                    raise Program_Error),
          when F_Max_Retries =>
             (case Fld is
   ...
   function Field_First (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Index is
     ((case Fld is
          ...
          when F_PIN =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Max_Retries
              then
   ...
   function Valid_Predecessor (Ctx : Context; Fld : Virtual_Field) return Boolean is
     ((case Fld is
          ...
          when F_PIN =>
             (Valid (Ctx.Cursors (F_Max_Retries))
              and Ctx.Cursors (Fld).Predecessor = F_Max_Retries),
          when F_Final =>
             (Valid (Ctx.Cursors (F_PIN))
   ...
   function Incomplete_Message (Ctx : Context) return Boolean is
     (Incomplete (Ctx, F_Max_Retries)
      or Incomplete (Ctx, F_PIN));

   function Get_Max_Retries (Ctx : Context) return RFLX.PIN_FSM.Retries is
     (To_Actual (Ctx.Cursors (F_Max_Retries).Value.Max_Retries_Value));
