...
package body RFLX.PIN_FSM.Request with
  SPARK_Mode
is
   ...
   begin
      Ctx := (Buffer_First, Buffer_Last, First, Last, First - 1, Buffer, (F_Kind => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
      Buffer := null;
   end Initialize;

   procedure Reset (Ctx : in out Context) is
   begin
      Ctx.Cursors := (F_Kind => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final));
      Ctx.Message_Last := Ctx.First - 1;
   end Reset;

   procedure Reset (Ctx : in out Context; First : RFLX_Types.Bit_Index; Last : RFLX_Types.Bit_Length) is
   begin
      Ctx := (Ctx.Buffer_First, Ctx.Buffer_Last, First, Last, First - 1, Ctx.Buffer, (F_Kind => (State => S_Invalid, Predecessor => F_Initial), others => (State => S_Invalid, Predecessor => F_Final)));
   end Reset;
          ...
          when F_Kind =>
             (if
              ...
              then
                 ...
                 RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Deauth))
              then
                 F_Final
              elsif
   ...
   is
      ...
      function Extract is new RFLX_Types.Extract (RFLX.PIN_FSM.Kind_Base);
   ...
   is
      ...
      function Offset return RFLX_Types.Offset is
        (RFLX_Types.Offset ((8 - Last mod 8) mod 8));
      procedure Insert is new RFLX_Types.Insert (RFLX.PIN_FSM.Kind_Base);
   ...
   procedure Set_Kind (Ctx : in out Context; Val : RFLX.PIN_FSM.Kind) is
      Field_Value : constant Field_Dependent_Value := (F_Kind, To_Base (Val));
