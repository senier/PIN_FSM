...
package RFLX.PIN_FSM.Request with
  SPARK_Mode,
  Annotate =>
...
is
   ...
   type Virtual_Field is (F_Initial, F_Kind, F_Data, F_PIN, F_Final);

   subtype Field is Virtual_Field range F_Kind .. F_PIN;
      ...
      record
         case Fld is
            when F_Initial | F_Data | F_Final =>
               null;
            when F_Kind =>
               Kind_Value : RFLX.PIN_FSM.Kind_Base;
            when F_PIN =>
               PIN_Value : RFLX.PIN_FSM.PIN;
         end case;
      end record;
   ...
   function Get_Kind (Ctx : Context) return RFLX.PIN_FSM.Kind with
     Pre =>
       Valid (Ctx, F_Kind);

   function Get_PIN (Ctx : Context) return RFLX.PIN_FSM.PIN with
     Pre =>
       Valid (Ctx, F_PIN);
   ...
   procedure Set_Kind (Ctx : in out Context; Val : RFLX.PIN_FSM.Kind) with
     Pre =>
       not Ctx'Constrained
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_Kind)
       and then Field_Condition (Ctx, (F_Kind, To_Base (Val)))
       and then True
       and then Available_Space (Ctx, F_Kind) >= Field_Size (Ctx, F_Kind),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_Kind)
       and Get_Kind (Ctx) = Val
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_Kind))
       and Invalid (Ctx, F_Data)
       and Invalid (Ctx, F_PIN)
       and (if
               RFLX_Types.U64 (To_Base (Get_Kind (Ctx))) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward))
            then
               Predecessor (Ctx, F_Data) = F_Kind
               and Valid_Next (Ctx, F_Data))
       and (if
               RFLX_Types.U64 (To_Base (Get_Kind (Ctx))) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
               or RFLX_Types.U64 (To_Base (Get_Kind (Ctx))) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change))
            then
               Predecessor (Ctx, F_PIN) = F_Kind
               and Valid_Next (Ctx, F_PIN))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
   ...
   procedure Set_PIN (Ctx : in out Context; Val : RFLX.PIN_FSM.PIN) with
     Pre =>
       not Ctx'Constrained
       and then Has_Buffer (Ctx)
       and then Valid_Next (Ctx, F_PIN)
       and then Field_Condition (Ctx, (F_PIN, To_Base (Val)))
       and then Valid (To_Base (Val))
       and then Available_Space (Ctx, F_PIN) >= Field_Size (Ctx, F_PIN),
     Post =>
       Has_Buffer (Ctx)
       and Valid (Ctx, F_PIN)
       and Get_PIN (Ctx) = Val
       and (if Structural_Valid_Message (Ctx) then Message_Last (Ctx) = Field_Last (Ctx, F_PIN))
       and Ctx.Buffer_First = Ctx.Buffer_First'Old
       and Ctx.Buffer_Last = Ctx.Buffer_Last'Old
       and Ctx.First = Ctx.First'Old
       and Ctx.Last = Ctx.Last'Old
       and Predecessor (Ctx, F_PIN) = Predecessor (Ctx, F_PIN)'Old
       and Valid_Next (Ctx, F_PIN) = Valid_Next (Ctx, F_PIN)'Old
...
private
      ...
      and then ((if
                 ...
                 then
                    (Valid (Cursors (F_Kind))
                     and then Cursors (F_Data).Predecessor = F_Kind
                     and then RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward))))
                and then (if
                             Structural_Valid (Cursors (F_PIN))
                          then
                             (Valid (Cursors (F_Kind))
                              and then Cursors (F_PIN).Predecessor = F_Kind
                              and then (RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
                                        or RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change))))))
      and then ((if Invalid (Cursors (F_Kind)) then Invalid (Cursors (F_Data)))
                and then (if Invalid (Cursors (F_Kind)) then Invalid (Cursors (F_PIN))))
      and then (if
                   Structural_Valid (Cursors (F_Kind))
                then
                   Cursors (F_Kind).Last - Cursors (F_Kind).First + 1 = RFLX.PIN_FSM.Kind_Base'Size
                   ...
                   and then (if
                                ...
                                and then RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward))
                             then
                                Cursors (F_Data).Last - Cursors (F_Data).First + 1 = RFLX_Types.Bit_Length (Last) - RFLX_Types.Bit_Length (Cursors (F_Kind).Last)
                                and then Cursors (F_Data).Predecessor = F_Kind
                                and then Cursors (F_Data).First = Cursors (F_Kind).Last + 1)
                   and then (if
                                Structural_Valid (Cursors (F_PIN))
                                and then (RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
                                          or RFLX_Types.U64 (Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change)))
                             then
                                Cursors (F_PIN).Last - Cursors (F_PIN).First + 1 = RFLX.PIN_FSM.PIN'Size
                                and then Cursors (F_PIN).Predecessor = F_Kind
                                and then Cursors (F_PIN).First = Cursors (F_Kind).Last + 1)));
   ...
   function Path_Condition (Ctx : Context; Fld : Field) return Boolean is
     ((case Ctx.Cursors (Fld).Predecessor is
          ...
          when F_Kind =>
             (case Fld is
                 when F_Data =>
                    RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward)),
                 when F_PIN =>
                    RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
                    or RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change)),
                 when others =>
                    False),
          when F_Data | F_PIN | F_Final =>
             False));
   ...
   function Field_Condition (Ctx : Context; Val : Field_Dependent_Value) return Boolean is
     ((case Val.Fld is
          when F_Initial =>
             True,
          when F_Kind =>
             RFLX_Types.U64 (Val.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward))
             or RFLX_Types.U64 (Val.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Deauth))
             or RFLX_Types.U64 (Val.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
             or RFLX_Types.U64 (Val.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change)),
          when F_Data | F_PIN =>
             True,
          when F_Final =>
             False));
   ...
   function Field_Size (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Length is
     ((case Ctx.Cursors (Fld).Predecessor is
          when F_Initial =>
             (case Fld is
                 when F_Kind =>
                    RFLX.PIN_FSM.Kind_Base'Size,
                 when others =>
                    raise Program_Error),
          when F_Kind =>
             (case Fld is
                 when F_Data =>
                    RFLX_Types.Bit_Length (Ctx.Last) - RFLX_Types.Bit_Length (Ctx.Cursors (F_Kind).Last),
                 when F_PIN =>
                    RFLX.PIN_FSM.PIN'Size,
                 when others =>
                    raise Program_Error),
          when F_Data | F_PIN | F_Final =>
             0));
   ...
   function Field_First (Ctx : Context; Fld : Field) return RFLX_Types.Bit_Index is
     ((case Fld is
          ...
          when F_Data =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Kind
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward))
              then
          ...
          when F_PIN =>
             (if
                 Ctx.Cursors (Fld).Predecessor = F_Kind
                 and then (RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
                           or RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change)))
              then
                 Ctx.Cursors (Ctx.Cursors (Fld).Predecessor).Last + 1
              else
   ...
   function Valid_Predecessor (Ctx : Context; Fld : Virtual_Field) return Boolean is
     ((case Fld is
          ...
          when F_Final =>
             ...
             or (Valid (Ctx.Cursors (F_Kind))
                 and Ctx.Cursors (Fld).Predecessor = F_Kind)
             or (Valid (Ctx.Cursors (F_PIN))
                 and Ctx.Cursors (Fld).Predecessor = F_PIN)));
   ...
   function Structural_Valid_Message (Ctx : Context) return Boolean is
     (Valid (Ctx, F_Kind)
      and then ((Structural_Valid (Ctx, F_Data)
                 and then RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Forward)))
                or RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Deauth))
                or (Valid (Ctx, F_PIN)
                    and then (RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Auth))
                              or RFLX_Types.U64 (Ctx.Cursors (F_Kind).Value.Kind_Value) = RFLX_Types.U64 (To_Base (RFLX.PIN_FSM.Change))))));
   ...
   function Incomplete_Message (Ctx : Context) return Boolean is
     (Incomplete (Ctx, F_Kind)
      ...
      or Incomplete (Ctx, F_PIN));

   function Get_Kind (Ctx : Context) return RFLX.PIN_FSM.Kind is
     (To_Actual (Ctx.Cursors (F_Kind).Value.Kind_Value));

   function Get_PIN (Ctx : Context) return RFLX.PIN_FSM.PIN is
     (To_Actual (Ctx.Cursors (F_PIN).Value.PIN_Value));
